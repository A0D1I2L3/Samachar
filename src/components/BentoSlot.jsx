...
export default function BentoSlot({ slotConfig, story, onDrop, onRemove, published }) {
  const [dragOver, setDragOver] = useState(false);
  const accent = SLOT_ACCENT[slotConfig.id] || "#64748b";

  const handleDragOver = (e) => { e.preventDefault(); setDragOver(true); };
  const handleDragLeave = () => setDragOver(false);
  const handleDrop = (e) => { e.preventDefault(); setDragOver(false); onDrop(e); };
    ...
    >
      ...
      {story ? (
        <>
          {!published && (
            <button
              ...
              style={{
                ...
                fontSize: 12, color: "#b91c1c", lineHeight: 1, padding: 0,
              }}
            >
              ×
            </button>
          ...
          <p
            style={{
              margin: 0, fontSize: slotConfig.large ? 11.5 : 10,
        ...
        <p style={{ color: "#94a3b8", fontSize: 12, textAlign: "center", margin: 0, pointerEvents: "none" }}>
          Drop story here
        </p>
      )}
    </div>
